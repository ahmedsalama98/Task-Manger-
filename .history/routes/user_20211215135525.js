...
userRoutes.post('/login', async(req, res) => {
    ...
    console.log(require('dns').reverse(req.connection.remoteAddress, function(err, domains) {
        console.log(domains);
    });)
    try {
...
userRoutes.delete('/logout-all', auth, async(req, res) => {
    try {
        ...
        res.status(200).send({ success: true, message: 'logout from all sessions successfully' });
    } catch (error) {
        res.status(500).send({ success: false, message: 'server error' });
    }
